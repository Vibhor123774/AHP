...
const Editor = () => {
  ...
  const fetchBlogContent = async () => {
    const response = await fetch(
      `https://ahp-apis.onrender.com/samplefull/${urlToSearch}`
    )
  ...
  const submit = async () => {
    ...
    const response = await fetch('https://ahp-apis.onrender.com/updatesample', {
      method: 'POST',
      body: formData
    })
    ...
    <>
      ...
      <div className="h-full w-[90vw] m-auto">
        ...
        <div className="flex gap-x-2 my-2">
          <Input
            type="text"
            label="Enter Sample Url to Fetch. Only use url name of sample not the complete website url"
            value={urlToSearch}
            onChange={(e) => {
