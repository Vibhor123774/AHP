...
const Editor = () => {
  ...
  const submit = async () => {
    ...
    const response = await fetch('https://ahp-apis.onrender.com/newblog', {
      method: 'POST',
      body: formData
    })
    if (response.status === 200) {
      ...
      alert('Blog with the same URL already exist')
    } else {
  ...
  const updateBlogUrl = (blogTitle) => {
    const strWithoutSpecialChars = blogTitle.replace(/[^a-zA-Z0-9 ]/g, '')
